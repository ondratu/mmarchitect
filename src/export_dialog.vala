...
public class ExportDialog : Gtk.FileChooserDialog {
    ...
    public void set_path (string filepath = "") {
        if (filepath == "") {
            ...
            set_current_folder (Path.get_dirname (filepath));
            string fname = Path.get_basename (filepath);
    ...
    public void on_response (int response_id) {
        ...
        if (filter == txt) {
            ...
            if (!fname.down ().has_suffix (".htm")
                    && !fname.down ().has_suffix (".html"))
            {
            ...
            if (!fname.down ().has_suffix (".dhtm")
                    && !fname.down ().has_suffix (".dhtml"))
            {
