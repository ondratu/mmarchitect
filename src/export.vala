...
namespace Exporter {
    ...
    void write_mm_node (Xml.TextWriter w, Node node, bool parent_is_root) {
        foreach (var n in node.children) {
    ...
    void cairo_get_translation (int width, int height, Node node,
            out double x, out double y)
    {
        int dist = (node.full_right.width - node.full_left.width).abs () / 2;

        if (node.full_right.width > node.full_left.width) {
            x = Math.lrint (width / 2 - dist ) + 0.5;
        } else {
            x = Math.lrint (width / 2 + dist ) + 0.5;
        }

        y = Math.lrint (height / 2 - node.get_higher_full () / 2) + 0.5;
    }
